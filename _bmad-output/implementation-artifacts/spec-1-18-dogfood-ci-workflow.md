...
- reviewed_range: a5bde87f..9004c398, verified against HEAD
- forced_area: full-history checkout is present before the merge-base review; the exclusion declaration and denominator share one predicate; dogfood remains PR-only and uploads on failure.
- findings_fixed_and_verified_at_HEAD:
  ...
  - remediation_evidence: `scripts/ci-workflow.test.mjs`; focused regression suite passed 2026-08-01.
  - [medium] .github/workflows/ci.yml:1-90 — the workflow does not declare a minimal `permissions` block. The dogfood job only needs repository contents read access, but on repositories whose Actions default is read/write the `GITHUB_TOKEN` retains unnecessary write capability while third-party action code and project build/test scripts execute. Add top-level `permissions: contents: read` (and only any additional permission actually required).
  - [low] .github/workflows/ci.yml:19,23,30,83 — all actions, including the newly added artifact uploader, are referenced by mutable major tags rather than immutable commit SHAs. A compromised/moved tag gains the job's token and source checkout. Pin exact SHAs and use comments/automation for version tracking, especially now that this workflow is the project's own security gate.
